...
def bootStrapSubscriber(event):
    ...
    if not "host_vmuuid_index" in instUtilityICatalog.keys():
        host_vmuuid_index = TextIndex(interface=ISearchableText,
                                        field_name='getSearchableHostVmUuid',
                                        field_callable=True)
        instUtilityICatalog['host_vmuuid_index'] = host_vmuuid_index
        # search for IAdmUtilSupervisor
        utils = [ util for util in sitem.registeredUtilities()
                  if util.provided.isOrExtends(IAdmUtilSupervisor)]
        instAdmUtilSupervisor = utils[0].component
        instAdmUtilSupervisor.appendEventHistory(\
            u" bootstrap: ICatalog - create vm uuid index for entry type 'vmware_vm'")
