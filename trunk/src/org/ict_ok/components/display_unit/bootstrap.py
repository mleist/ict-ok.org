...
logger = logging.getLogger("Compon. DisplayUnit")
...
def createUtils(root_folder, connection=None, dummy_db=None):
    ...
    if not "display_unit_oid_index" in instUtilityICatalog.keys():
        display_unit_oid_index = KeywordIndex(interface=ISearchableText,
                                        field_name='getSearchableDisplayUnitOid',
                                        field_callable=True)
        instUtilityICatalog['display_unit_oid_index'] = display_unit_oid_index
        # search for IAdmUtilSupervisor
        utils = [ util for util in sitem.registeredUtilities()
                  if util.provided.isOrExtends(IAdmUtilSupervisor)]
        instAdmUtilSupervisor = utils[0].component
        instAdmUtilSupervisor.appendEventHistory(\
            u" bootstrap: ICatalog - create index for entry type 'display_unit'")
        instAdmUtilSupervisor.appendEventHistory(\
            u" bootstrap: ICatalog - create index for entry type 'appsoftware'")

    ensureComponentFolderOnBootstrap(IDisplayUnitFolder,
                 u"DisplayUnits",
                 u'org.ict_ok.components.display_unit.display_unit.DisplayUnitFolder',
                 root_folder,
                 sitem)
