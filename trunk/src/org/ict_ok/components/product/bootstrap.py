...
logger = logging.getLogger("Compon. Product")
...
def createUtils(root_folder, connection=None, dummy_db=None):
    ...
    if not "product_oid_index" in instUtilityICatalog.keys():
        product_oid_index = KeywordIndex(interface=ISearchableText,
                                        field_name='getSearchableProductOid',
                                        field_callable=True)
        instUtilityICatalog['product_oid_index'] = product_oid_index
        # search for IAdmUtilSupervisor
        utils = [ util for util in sitem.registeredUtilities()
                  if util.provided.isOrExtends(IAdmUtilSupervisor)]
        instAdmUtilSupervisor = utils[0].component
        instAdmUtilSupervisor.appendEventHistory(\
            u" bootstrap: ICatalog - create index for entry type 'product'")
        instAdmUtilSupervisor.appendEventHistory(\
            u" bootstrap: ICatalog - create index for entry type 'appsoftware'")

    ensureComponentFolderOnBootstrap(IProductFolder,
                 u"Products",
                 u'org.ict_ok.components.product.product.ProductFolder',
                 root_folder,
                 sitem)
