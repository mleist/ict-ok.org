...
logger = logging.getLogger("Compon. WorkOrder")
...
def createUtils(root_folder, connection=None, dummy_db=None):
    ...
    if not "work_order_oid_index" in instUtilityICatalog.keys():
        work_order_oid_index = KeywordIndex(interface=ISearchableText,
                                        field_name='getSearchableWorkOrderOid',
                                        field_callable=True)
        instUtilityICatalog['work_order_oid_index'] = work_order_oid_index
        # search for IAdmUtilSupervisor
        utils = [ util for util in sitem.registeredUtilities()
                  if util.provided.isOrExtends(IAdmUtilSupervisor)]
        instAdmUtilSupervisor = utils[0].component
        instAdmUtilSupervisor.appendEventHistory(\
            u" bootstrap: ICatalog - create index for entry type 'work_order'")
        instAdmUtilSupervisor.appendEventHistory(\
            u" bootstrap: ICatalog - create index for entry type 'appsoftware'")

    ensureComponentFolderOnBootstrap(IWorkOrderFolder,
                 u"WorkOrders",
                 u'org.ict_ok.components.work_order.work_order.WorkOrderFolder',
                 root_folder,
                 sitem)
