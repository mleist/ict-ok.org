...
logger = logging.getLogger("Compon. Group")
...
def createUtils(root_folder, connection=None, dummy_db=None):
    ...
    if not "group_oid_index" in instUtilityICatalog.keys():
        group_oid_index = KeywordIndex(interface=ISearchableText,
                                        field_name='getSearchableGroupOid',
                                        field_callable=True)
        instUtilityICatalog['group_oid_index'] = group_oid_index
        # search for IAdmUtilSupervisor
        utils = [ util for util in sitem.registeredUtilities()
                  if util.provided.isOrExtends(IAdmUtilSupervisor)]
        instAdmUtilSupervisor = utils[0].component
        instAdmUtilSupervisor.appendEventHistory(\
            u" bootstrap: ICatalog - create index for entry type 'group'")
        instAdmUtilSupervisor.appendEventHistory(\
            u" bootstrap: ICatalog - create index for entry type 'appsoftware'")

    ensureComponentFolderOnBootstrap(IGroupFolder,
                 u"Groups",
                 u'org.ict_ok.components.group.group.GroupFolder',
                 root_folder,
                 sitem)
