...
logger = logging.getLogger("Compon. Address")
...
def createUtils(root_folder, connection=None, dummy_db=None):
    ...
    if not "address_oid_index" in instUtilityICatalog.keys():
        address_oid_index = KeywordIndex(interface=ISearchableText,
                                        field_name='getSearchableAddressOid',
                                        field_callable=True)
        instUtilityICatalog['address_oid_index'] = address_oid_index
        # search for IAdmUtilSupervisor
        utils = [ util for util in sitem.registeredUtilities()
                  if util.provided.isOrExtends(IAdmUtilSupervisor)]
        instAdmUtilSupervisor = utils[0].component
        instAdmUtilSupervisor.appendEventHistory(\
            u" bootstrap: ICatalog - create index for entry type 'address'")
        instAdmUtilSupervisor.appendEventHistory(\
