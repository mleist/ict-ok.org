...
logger = logging.getLogger("Compon. Role")
...
def createUtils(root_folder, connection=None, dummy_db=None):
    ...
    if not "role_oid_index" in instUtilityICatalog.keys():
        role_oid_index = KeywordIndex(interface=ISearchableText,
                                        field_name='getSearchableRoleOid',
                                        field_callable=True)
        instUtilityICatalog['role_oid_index'] = role_oid_index
        # search for IAdmUtilSupervisor
        utils = [ util for util in sitem.registeredUtilities()
                  if util.provided.isOrExtends(IAdmUtilSupervisor)]
        instAdmUtilSupervisor = utils[0].component
        instAdmUtilSupervisor.appendEventHistory(\
            u" bootstrap: ICatalog - create index for entry type 'role'")
        instAdmUtilSupervisor.appendEventHistory(\
            u" bootstrap: ICatalog - create index for entry type 'appsoftware'")

    ensureComponentFolderOnBootstrap(IRoleFolder,
                 u"Roles",
                 u'org.ict_ok.components.role.role.RoleFolder',
                 root_folder,
                 sitem)
