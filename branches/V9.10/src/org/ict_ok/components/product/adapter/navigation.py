...
class Navigation(SuperNavigation):
    ...
    def getContextObjList(self, preList=None, postList=None):
        ...
        if self.context.mainProduct != None:
            retList.append(('mainProduct', _(u'Main product'), self.context))
        if len(self.context.subProducts) > 0:
            retList.append(('subProducts', _(u'Sub products'), self.context))
        if len(self.context.workOrder) > 0:
