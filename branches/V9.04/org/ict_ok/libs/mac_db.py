...
class MacDB():
    def __init__(self):
        f = open("oui.txt", "r") #oui: http://standards.ieee.org/regauth/oui/oui.txt
        self._data = IOBTree()
        last_mac = None
        EOF = False
        re_mac = re.compile(r"[A-Z0-9]*\-[A-Z0-9]*\-[A-Z0-9]*")
        while not EOF:
            line = f.readline()
            if re.match(re_mac, line):
                last_mac = int(line[:8].replace("-", ""), 16)
                index = line.rfind("\t")
                org = line[index+1:].strip()
                self._data[last_mac] = {"short": org, "other":[]}
            elif last_mac != None:
                index = line.rfind("\t")
                if index != None and line != "\n":
                    other = line[index+1:].strip()
                    self._data[last_mac]["other"].append(other)
                if (line == "\n"):
                    country_index = len(self._data[last_mac]["other"])-1
    ...
    def getOrganization(self, mac_string):
        int_mac = int(mac_string[:8].replace(":", ""), 16)
        try:
