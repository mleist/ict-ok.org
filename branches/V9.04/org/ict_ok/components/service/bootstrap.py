...
def bootStrapSubscriber(event):
    ...
    if not "service_oid_index" in instUtilityICatalog.keys():
        service_oid_index = TextIndex(interface=ISearchableText,
                                      field_name='getSearchableServiceOid',
                                      field_callable=True)
        instUtilityICatalog['service_oid_index'] = service_oid_index
    ...
    folderName = u"Services"
    if folderName not in root_folder.keys():
        #newFolder = TestComponentFolder()
        newFolder = createObject(u'org.ict_ok.components.service.service.ServiceFolder')
