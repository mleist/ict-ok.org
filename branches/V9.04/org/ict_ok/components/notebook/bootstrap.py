...
logger = logging.getLogger("Compon. Notebook")
...
def bootStrapSubscriber(event):
    ...
    if not "notebook_oid_index" in instUtilityICatalog.keys():
        notebook_oid_index = TextIndex(interface=ISearchableText,
                                        field_name='getSearchableNotebookOid',
                                        field_callable=True)
        instUtilityICatalog['notebook_oid_index'] = notebook_oid_index
        # search for IAdmUtilSupervisor
        utils = [ util for util in sitem.registeredUtilities()
                  if util.provided.isOrExtends(IAdmUtilSupervisor)]
        instAdmUtilSupervisor = utils[0].component
        instAdmUtilSupervisor.appendEventHistory(\
            u" bootstrap: ICatalog - create index for entry type 'notebook'")
        
    folderName = u"Notebooks"
    if folderName not in root_folder.keys():
        #newFolder = TestComponentFolder()
        newFolder = createObject(u'org.ict_ok.components.notebook.notebook.NotebookFolder')
