...
if os.path.exists('src/org/ict_ok/components/device/wf/devicenagios.xpdl'):
    xpdl_filename = 'src/org/ict_ok/components/device/wf/devicenagios.xpdl'
else:
    xpdl_filename = os.getenv("ICT_DEVICE_XPDL")
...
integration.dev_nagios1_par1Participant = ParticipantICT_Ok


class ParticipantNagios(Participant):
    def __init__(self, activity):
        Participant.__init__(self, activity)
integration.dev_nagios1_par2Participant = ParticipantNagios
...
class ApplicationBase(Persistent, Location):
    ...
    def start(self):
        ...
        deviceObj = getattr(wfd, 'object')
        if self not in deviceObj.wf_worklist:
            deviceObj.wf_worklist.append(self)
            deviceObj._p_changed = True

    def finish(self):
        wfd = self.participant.activity.process.workflowRelevantData
        deviceObj = getattr(wfd, 'object')
        if self in deviceObj.wf_worklist:
            deviceObj.wf_worklist.remove(self)
            deviceObj._p_changed = True
...
class NagiosTriggerOnlineWorkItem(ApplicationBase):
    ...
    def change(self):
        ...
        if getattr(wfd, 'state') != getattr(wfd, 'new_state'):
            ...
            deviceObj = getattr(wfd, 'object')
            deviceObj.appendHistoryEntry(change_str, level=u"warn")
            self.finish()
    def finish(self):
        ...
        wfd = self.participant.activity.process.workflowRelevantData
        deviceObj = getattr(wfd, 'object')
        #notify(NotifyUserEvent(deviceObj))
        ApplicationBase.finish(self)
