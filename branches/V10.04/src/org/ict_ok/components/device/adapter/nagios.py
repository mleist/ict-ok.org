...
class GenNagios(ParentGenNagios):
    ...
    def traverse4nagiosGeneratorPre(self, level=0, comments=True):
        ...
        ipAddressList = []
        interfaces = getattr(self.context, 'interfaces', None)
        if interfaces is not None and len(interfaces)>0:
            for interface in interfaces:
                ipAddresses = getattr(interface, 'ipAddresses', None)
                if ipAddresses is not None and len(ipAddresses)>0:
                    ipAddressList.append(ipAddresses[0])
        if self.wantsCheck():
            valueChanged = True
