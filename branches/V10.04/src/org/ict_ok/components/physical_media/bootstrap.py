...
def createUtils(root_folder, connection=None, dummy_db=None):
    ...
    if not "physical_media_oid_index" in instUtilityICatalog.keys():
        physical_media_oid_index = KeywordIndex(interface=ISearchableText,
                                        field_name='getSearchablePhysicalMediaOid',
                                        field_callable=True)
        instUtilityICatalog['physical_media_oid_index'] = physical_media_oid_index
        # search for IAdmUtilSupervisor
        utils = [ util for util in sitem.registeredUtilities()
                  if util.provided.isOrExtends(IAdmUtilSupervisor)]
        instAdmUtilSupervisor = utils[0].component
        instAdmUtilSupervisor.appendEventHistory(\
            u" bootstrap: ICatalog - create index for entry type 'physical_media'")
        instAdmUtilSupervisor.appendEventHistory(\
            u" bootstrap: ICatalog - create index for entry type 'appsoftware'")

    ensureComponentFolderOnBootstrap(IPhysicalMediaFolder,
                 u"PhysicalMedia",
                 u'org.ict_ok.components.physical_media.physical_media.PhysicalMediaFolder',
                 root_folder,
                 sitem)
