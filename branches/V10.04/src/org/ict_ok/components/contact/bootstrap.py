...
logger = logging.getLogger("Compon. Contact")
...
def createUtils(root_folder, connection=None, dummy_db=None):
    ...
    if not "contact_oid_index" in instUtilityICatalog.keys():
        contact_oid_index = KeywordIndex(interface=ISearchableText,
                                        field_name='getSearchableContactOid',
                                        field_callable=True)
        instUtilityICatalog['contact_oid_index'] = contact_oid_index
        # search for IAdmUtilSupervisor
        utils = [ util for util in sitem.registeredUtilities()
                  if util.provided.isOrExtends(IAdmUtilSupervisor)]
        instAdmUtilSupervisor = utils[0].component
        instAdmUtilSupervisor.appendEventHistory(\
            u" bootstrap: ICatalog - create index for entry type 'contact'")
        instAdmUtilSupervisor.appendEventHistory(\
            u" bootstrap: ICatalog - create index for entry type 'appsoftware'")

    ensureComponentFolderOnBootstrap(IContactFolder,
                 u"Contacts",
                 u'org.ict_ok.components.contact.contact.ContactFolder',
                 root_folder,
                 sitem)
