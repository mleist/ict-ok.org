...
logger = logging.getLogger("Compon. ContactItem")
...
def createUtils(root_folder, connection=None, dummy_db=None):
    ...
    if not "contact_item_oid_index" in instUtilityICatalog.keys():
        contact_item_oid_index = KeywordIndex(interface=ISearchableText,
                                        field_name='getSearchableContactItemOid',
                                        field_callable=True)
        instUtilityICatalog['contact_item_oid_index'] = contact_item_oid_index
        # search for IAdmUtilSupervisor
        utils = [ util for util in sitem.registeredUtilities()
                  if util.provided.isOrExtends(IAdmUtilSupervisor)]
        instAdmUtilSupervisor = utils[0].component
        instAdmUtilSupervisor.appendEventHistory(\
            u" bootstrap: ICatalog - create index for entry type 'contact_item'")
        instAdmUtilSupervisor.appendEventHistory(\
            u" bootstrap: ICatalog - create index for entry type 'appsoftware'")

    ensureComponentFolderOnBootstrap(IContactItemFolder,
                 u"ContactItems",
                 u'org.ict_ok.components.contact_item.contact_item.ContactItemFolder',
                 root_folder,
                 sitem)
