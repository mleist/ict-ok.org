...
logger = logging.getLogger("Compon. Printer")
...
def createUtils(root_folder, connection=None, dummy_db=None):
    ...
    if not "printer_oid_index" in instUtilityICatalog.keys():
        printer_oid_index = KeywordIndex(interface=ISearchableText,
                                        field_name='getSearchablePrinterOid',
                                        field_callable=True)
        instUtilityICatalog['printer_oid_index'] = printer_oid_index
        # search for IAdmUtilSupervisor
        utils = [ util for util in sitem.registeredUtilities()
                  if util.provided.isOrExtends(IAdmUtilSupervisor)]
        instAdmUtilSupervisor = utils[0].component
        instAdmUtilSupervisor.appendEventHistory(\
            u" bootstrap: ICatalog - create index for entry type 'printer'")

    ensureComponentFolderOnBootstrap(IPrinterFolder,
                 u"Printers",
                 u'org.ict_ok.components.printer.printer.PrinterFolder',
                 root_folder,
                 sitem)
